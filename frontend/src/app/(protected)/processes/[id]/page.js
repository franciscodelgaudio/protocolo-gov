...
import { updateProcessStatus } from '@/app/actions'
...
export default async function ProcessDetailPage({ params }) {
  ...
  return (
    <div className="max-w-xl">
      <div className="mb-6">
        <Link href="/processes" className="text-sm text-muted-foreground hover:underline">← Voltar</Link>
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusColors[process.status] || 'bg-gray-100 text-gray-800'}`}>
          {process.status || '—'}
        </span>
      </div>
      ...
      {process.requestId && (
        <div className="mt-4">
          <Link href={`/requests/${process.requestId}`} className="text-sm text-primary hover:underline">
